...
import { CloudArrowUp, Tv } from "react-bootstrap-icons";
...
const ChooseTvPage: IPage = Object.assign(
    () => {
        ...
        const lastVideos = Promise.all((tvs ?? []).map(tv => new PlayerApi(tv.id, bearer).GetLast()))
        const lastVideosMetadata = lastVideos.then(videos => Promise.all(videos.map(async video => {
            const metadata = await VideosApi.GetYouTubeMetadata(video.video.link).then(res => res.data);
        ...
        return (<div>
            <Center><h1>Текущи екрани</h1></Center>
            <Row>
                {tvs.map(tv => (
                    <Col xs={6} md={3} key={tv.id}>
                        <Card>
                            <Card.Img src={result?.find(v => v.video.queueId == tv.id)?.thumbnail_url ?? BrokenImageURL}></Card.Img>
                            <Card.Body>
    ...
    {
        checkAccess: (state: UserState): boolean => state.authed && !!state.user?.user.rights.includes(RightsEnum.ControllPlayer || RightsEnum.ChangeTv),
        path: '/tv',
        pageName: (<><Tv />Екрани</>),
        showable: true
    }
