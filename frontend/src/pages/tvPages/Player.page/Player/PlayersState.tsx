...
const PlayerStatesVis: FC<PlayerStates> = ({ socketApi }) => {
  ...
  return (
    <div>
      <Center>
        {" "}
        <Button onClick={() => socketApi.SendAction(Actions.AskForState)}>
          Получи състояния
        </Button>
      </Center>
      {answers.map((answ, ind) => {
        return (
          <VideoBox videoUrl={answ.CurrentVideo.video.link} key={answ.Seconds}>
            <div>
              <p>
                Звукът е {answ.IsMuted ? "изключен" : "включен"} и е{" "}
                {answ.Volume}
              </p>
...
export default memo(PlayerStatesVis);
