...
const UsersTable: FC<UsersTableProps> = ({ searchPattern }) => {
  ...
  const { result, isLoading, error } = useFetching(
    UsersApi.GetUsers(token, searchPattern, usersPerPage * page, usersPerPage),
    [searchPattern, page]
  );
  ...
  const isThereMorePages = users?.length === usersPerPage;
  ...
  return (
    <>
      ...
      <Row className="justify-content-between">
        ...
        {isThereMorePages && (
          <Button className="w-25" onClick={() => setPage(page + 1)}>
            Следваща страница
          </Button>
        )}
      </Row>
