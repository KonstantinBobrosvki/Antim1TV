...
const ModerateVideosPage: IPage = Object.assign(() => {
    ...
    const filtredResults = useMemo(() => {
        if (isLoading || error)
            return [];
        return result!.filter(el => true !== moderated.some(broken => broken === el.id))
    ...
    const AllowVideo = (videoId: number) => {
        VideosApi.AllowVideo(videoId, accses).then(() => {
            dispatch(alertsSlice.actions.add({ type: 'info', message: 'Успешно позволено' }))
            setModerated([videoId, ...moderated])
        })
    }
    ...
    const onRefreshClick = () => {
        ...
        dispatch(alertsSlice.actions.add({ type: 'info', message: 'Подновихте видеата ' + (refreshCount + 1) + " пъти! Браво!" }))
    ...
    if (isLoading) {
        return (
            <>
                <Center><TextLoader>Видеата за проверка зареждат</TextLoader></Center>
                <Center>
    ...
    if (error) {
        dispatch(alertsSlice.actions.add({ type: 'danger', message: (error.body as any).message }))
        return (
            <Center> <h1>Възникна грешка. Опитайте отново</h1></Center>
        )
    }
    return (<Col sm={12}>
        <Center> <h2>Видеа за проверка</h2> </Center>
        <Row>
            ...
            {filtredResults?.map(video => {
                return (<Col sm={6} lg={4} className='my-3' key={video.id}>
                    <VideoBox videoUrl={video.link} onFailLoad={() => RejectVideo(video.id)}>
                        ...
                        <Row>
                            <Col>
                                <Button variant='info' onClick={() => AllowVideo(video.id)}>Позволи</Button>
                            </Col>
                            <Col>
                                <Button variant='danger' onClick={() => RejectVideo(video.id)}>Изтрий</Button>
                            </Col>
                        </Row>
                    </VideoBox>
                </Col>)
        ...
        <Center>
            ...
            <Button variant="primary" className="mt-5" onClick={onRefreshClick}>Поднови</Button>
