...
type PawType = {
    guid: number,
    up: ReturnType<typeof document.querySelector>,
    down: ReturnType<typeof document.querySelector>
}

const NotFoundPage = Object.assign(() => {
    ...
    useEffect(() => {
        ...
        let timeouts: any = {};
        const ChagePaw = (paw: PawType) => {
            let interaval = Math.random() * 1100 + 400;
            paw!.up!.classList.toggle('visibility-hide');
            paw!.down!.classList.toggle('visibility-hide');
            timeouts[paw.guid] = setTimeout(function () {
                ChagePaw(paw)
            }, interaval);
        }


        Object.keys(cat).map(pawName => {
            const paw = (cat as any)[pawName];
            ChagePaw({
                guid: Math.floor(Math.random() * 100 + Date.now()),
                up: document.querySelector(paw.up),
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 783.55 354.91">
                        <g id="bongo-cat">
                            ...
                            <g className="paw paw-right">
                                <path className="down visibility-hide"
