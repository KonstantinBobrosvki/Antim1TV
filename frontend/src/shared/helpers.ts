...
export function ExcepionHandler(dispatch: ReturnType<typeof useAppDispatch>) {
    return (err: any) => {
        ...
        if (error.response) {
            ...
            dispatch(alertsSlice.actions.add({ type: 'warning', message: 'Не е получен отговор от сървъра' }))

            console.log(error.request);
        } else {

            dispatch(alertsSlice.actions.add({ type: 'warning', message: 'Не успяхме да пратим запитване до сървъра' }))
